...
import { ADD_PROFILE_IMAGE_ROUTE, HOST, REMOVE_PROFILE_IMAGE_ROUTE, UPDATE_PROFLE_ROUTE } from "@/utils/constants";
...
function Profile() {
  ...
  const validateProfile = () => {
    if (!firstName) {
      toast.error("Bhau First Name is Required.");
      return false;
    }
    if (!lastName) {
      toast.error("Bhau Last Name is Required.");
      return false;
    }
    return true;
  };
    ...
    if (validateProfile()) {
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          toast.success("Bhau Your Profile is Updated Successfully.");
          navigate("/chat");
        }
      } catch (error) {
        console.log(error);
  ...
  return (
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10 rounded-2xl ">
      <div className=" w-[80vw] md:w-max flex flex-col gap-10">

        <div className="">
          <IoArrowBack className="text-4xl lg:text-6xl text-white text-opacity-90 cursor-pointer" onClick={handleNavigate} />
        </div>
